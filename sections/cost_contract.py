...
# Use canonical versions from utils.helpers (eliminates duplication)
...
WORKFLOW_DETAILS = {
    ...
    "Cost anomaly detection": "Statistical Z-score anomaly detection with automated incident correlation.",
    "Chargeback allocation": "Owner-based and database-based credit allocation with multi-rate pricing.",
    "Network and transfer costs": "Data transfer, PrivateLink, replication, and egress cost visibility.",
...
WORKFLOW_MODULES = {
    ...
    "Cost anomaly detection": "sections.cost_anomalies",
    "Chargeback allocation": "sections.cost_chargeback",
    "Network and transfer costs": "sections.cost_network",
}
...
def _build_cost_cockpit_sql_dt(company: str, days: int) -> str:
    """Cost cockpit SQL reading from Dynamic Table views (preferred, near-zero cost)."""
    wh_filter = get_wh_filter_clause("warehouse_name", company)
    return f"""
    WITH current_period AS (
        SELECT warehouse_name, SUM(total_credits) AS credits
    ...
    prior_period AS (
        SELECT warehouse_name, SUM(total_credits) AS credits
        FROM DBA_MAINT_DB.OVERWATCH.V_OVERWATCH_DAILY_CREDITS
        WHERE usage_date >= DATEADD('day', -{int(days) * 2}, CURRENT_DATE())
          AND usage_date < DATEADD('day', -{int(days)}, CURRENT_DATE())
          {wh_filter}
        GROUP BY warehouse_name
    ),
    deltas AS (
        ...
        FULL OUTER JOIN prior_period p ON c.warehouse_name = p.warehouse_name
    )
    SELECT
...
def _render_cost_watch_floor(company: str, credit_price: float) -> None:
    ...
    if st.button("Load Full Cost Proof", key="cost_contract_cockpit_load", type="primary"):
            ...
            if session is None:
                return
            try:
                _cockpit_result = run_query(
                    build_mart_cost_cockpit_sql(company, int(days)),
                    ttl_key=f"cost_contract_cockpit_mart_{company}_{days}",
                    tier="historical",
                    section="Cost & Contract",
                )
                if _cockpit_result is not None and not _cockpit_result.empty:
                    st.session_state["cost_contract_cockpit"] = _cockpit_result
                    ...
                    precompute_shell_metrics("cost_contract_cockpit", _cockpit_result)
                else:
                    raise ValueError("Mart returned empty")
            except Exception as mart_exc:
                try:
                    _cockpit_result = run_query(
                        _build_cost_cockpit_sql(company, int(days)),
                    ...
                    st.session_state["cost_contract_cockpit_source"] = (
                        "Live: SNOWFLAKE.ACCOUNT_USAGE.WAREHOUSE_METERING_HISTORY"
                    )
                    st.session_state["cost_contract_cockpit_meta"] = {"company": company, "days": int(days)}
                    st.session_state["cost_contract_cockpit_error"] = ""
                    from utils.perf import precompute_shell_metrics
                    ...
                    st.session_state["cost_contract_cockpit_error"] = (
                        f"Historical data unavailable: {format_snowflake_error(exc)}"
                    )
