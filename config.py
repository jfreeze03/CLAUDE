...
SECTION_DEFINITIONS = (
    SectionDefinition("EXECUTIVE", "briefcase", "Executive Landing", "sections.executive_landing_shell"),
    SectionDefinition("EXECUTIVE", "bell", "Alert Center", "sections.alert_center_shell"),
    SectionDefinition("OPERATIONS", "target", "DBA Control Room", "sections.dba_control_room_shell"),
    ...
    SectionDefinition("COST & GOVERNANCE", "cost", "Cost & Contract", "sections.cost_contract_shell"),
    SectionDefinition("COST & GOVERNANCE", "security", "Security Posture", "sections.security_posture_shell"),
    SectionDefinition("COST & GOVERNANCE", "change", "Change & Drift", "sections.change_drift_shell"),
    SectionDefinition("PLATFORM", "home", "Account Health", "sections.account_health_shell"),
    SectionDefinition("PLATFORM", "map", "Architecture Readiness", "sections.architecture_readiness_shell"),
)
...
def resolve_role_profile(role: str) -> str:
    ...
    Unknown roles default to ANALYST (read-only navigation) rather than DBA
    to follow least-privilege principles. Privileged access requires an
    explicit match in ROLE_PROFILE_OVERRIDES or a recognized naming pattern.
    """
    ...
    if normalized.endswith("_PDMWMGMT") or "_PDMWMGMT_" in normalized:
        return "EXECUTIVE"
    if "ACCOUNTADMIN" in normalized or "SYSADMIN" in normalized:
        return "DBA"
