...
# Keys that should survive company switches because they are account-level
# or user-configured, not company-scoped.
_CROSS_COMPANY_PRESERVE_PREFIXES = (
    "_platform_health_score",
    "_contract_",
    "_overwatch_perf_",
    "_overwatch_saved_views",
    "sf_session",
    "_sf_session",
    "_overwatch_connection",
    "_overwatch_current_role",
    "_overwatch_role_defaults",
    "_overwatch_sidebar_panel",
    "_overwatch_dev_",
    "_logging_",
    "_query_logging_",
    "_detailed_query_tags_",
)
...
def invalidate_company_cache(
    *,
    ...
    Preserves settings, theme, navigation, triage filters, and cross-company
    state (health score, contract config, session, connection status).
    ...
    keys_to_drop = [
        ...
        and not any(k.startswith(p) for p in _CROSS_COMPANY_PRESERVE_PREFIXES)
