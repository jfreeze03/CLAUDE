...
from utils.security_hardening import inject_csp_headers
inject_csp_headers()
...
def _maybe_clear_scope_cache_on_filter_change() -> None:
    ...
    if previous_filter_signature is None:
        ...
        # Use partial invalidation — only clear date-dependent data
        # Preserves metadata, column probes, and static architecture objectives
        from utils.cache import clear_date_dependent_cache
        clear_date_dependent_cache()
        st.session_state["_prev_global_filter_signature"] = current_filter_signature
...
def _render_app_header(section: str, company: str, credit_price: float, role: str) -> None:
    ...
    left, center, right = st.columns([4.2, 1.8, 1.0])
    with left:
        st.markdown(
            f"""
            <div class="ow-topbar">
        ...
        from utils.breadcrumb import render_breadcrumb
        render_breadcrumb()
    with center:
        from utils.health_score import render_health_score_badge
        render_health_score_badge()
        from utils.notifications import render_notification_badge
    ...
    with right:
        st.markdown(
            ...
            <div class="ow-run-context">
                ...
                <div>{safe_role}</div>
                <div>${float(credit_price):.2f}/cr</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
...
from utils.admin import initialize_admin_actions_default
...
# Attempt dynamic company config load from OVERWATCH_COMPANY_SCOPE table
if connection_available and not st.session_state.get("_overwatch_dynamic_company_config_loaded"):
    try:
        from utils.company_config_loader import get_dynamic_company_config, merge_with_static_config
        _dynamic_config = get_dynamic_company_config(st.session_state.get("sf_session"))
        if _dynamic_config:
            config_module.COMPANY_CONFIG = merge_with_static_config(COMPANY_CONFIG, _dynamic_config)
    except Exception:
        pass

# Load persistent user preferences on first render
if connection_available and not st.session_state.get("_overwatch_user_prefs_loaded"):
    try:
        from utils.user_preferences import load_user_preferences, apply_preferences
        _prefs = load_user_preferences(st.session_state.get("sf_session"))
        if _prefs:
            apply_preferences(_prefs)
    except Exception:
        pass
...
with st.sidebar:
    ...
    if _sidebar_panel_toggle("🔔 Notifications", "notifications"):
        from utils.notifications import render_notification_panel
    ...
    if _sidebar_panel_toggle("Settings", "settings"):
        ...
        st.divider()
        st.caption("Contract Capacity (enables burn forecast)")
        contract_total = st.number_input(
            "Total contract credits",
            min_value=0, max_value=100_000_000,
            value=int(st.session_state.get("_contract_total_credits", 0) or 0),
            step=10000,
            key="_contract_total_credits_input",
            help="Annual contract credit commitment. Set to 0 if unknown.",
        )
        if contract_total > 0:
            st.session_state["_contract_total_credits"] = contract_total
            contract_remaining = st.number_input(
                "Remaining contract credits",
                min_value=0, max_value=contract_total,
                value=int(st.session_state.get("_contract_remaining_credits", contract_total) or contract_total),
                step=5000,
                key="_contract_remaining_credits_input",
                help="Credits remaining in current contract period.",
            )
            st.session_state["_contract_remaining_credits"] = contract_remaining
        else:
            st.session_state.pop("_contract_total_credits", None)
            st.session_state.pop("_contract_remaining_credits", None)
        ...
        if connection_available:
            from utils.user_preferences import render_save_preferences_button
            try:
                render_save_preferences_button(get_session())
            except Exception:
                pass
