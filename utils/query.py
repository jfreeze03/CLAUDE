...
def _build_overwatch_query_tag(section: str, ttl_key: str, tier: str) -> str:
    """Build a query tag for section-level OVERWATCH cost attribution.

    Always includes the section name for warehouse cost attribution.
    Detailed mode adds company, tier, and perf run ID.
    """
    ...
    # Always tag with section for cost attribution
    base_tag = f"OVERWATCH:{section_label[:80]}"
    ...
    perf = _perf_run_id()
    tag = f"{base_tag}|{company[:24]}|{str(tier or 'recent')[:20]}"
    if perf:
...
@contextmanager
def _get_query_cache_lock(query_text: str, cache_context: str, cache_salt: str, tier: str):
    """No-op context manager. Streamlit is single-threaded per session.

    Previously used threading.Lock but this adds complexity without value
    in a single-threaded Streamlit environment. Kept as a context manager
    for backward compatibility with existing `with _get_query_cache_lock(...):`
    call sites.
    """
    yield
