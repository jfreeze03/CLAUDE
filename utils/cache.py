...
def clear_all_cache(
    *,
    ...
    # Also clear pre-computed shell metrics
    metrics_keys = [k for k in list(st.session_state.keys()) if k.endswith("__metrics") or k.endswith("__shell")]
...
# Metadata prefixes that don't depend on date/company filters
_DATE_INDEPENDENT_PREFIXES = (
    "_overwatch_available_columns",
    "_overwatch_unavailable_column_views",
    "_overwatch_column_probe",
    "_overwatch_qh_detail_exprs",
    "_overwatch_show_statement_cache",
    "_task_management_execution_context_cache",
    "arch_objectives_df",  # Static architecture objectives
    "arch_forward_controls",  # Static control register
)


def clear_date_dependent_cache() -> None:
    """Clear only date-dependent evidence, preserving metadata and static config.

    Called when the date range changes. Much faster than clear_all_cache()
    because it preserves warehouse lists, column probes, and architecture
    objectives that don't change with date selection.
    """
    transient_prefixes = (
        "cost_contract_", "cc_", "dba_control_room_",
        "alert_center_", "workload_operations_",
        "wh_", "warehouse_health_",
        "health_data", "morning_data",
        "security_posture_summary", "security_posture_exceptions",
        "change_drift_summary", "change_drift_exceptions",
        "_data_", "df_",
    )
    keys_to_remove = [
        k for k in list(st.session_state.keys())
        if any(k.startswith(p) for p in transient_prefixes)
        and not any(k.startswith(p) for p in _DATE_INDEPENDENT_PREFIXES)
        and k not in PRESERVE_STATE_EXACT
    ]
    for k in keys_to_remove:
        del st.session_state[k]

    # Clear shell metric caches too
    metrics_keys = [k for k in list(st.session_state.keys()) if k.endswith("__metrics") or k.endswith("__shell")]
    for k in metrics_keys:
        del st.session_state[k]
