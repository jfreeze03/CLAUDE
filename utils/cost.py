...
def credits_to_dollars(credits: float, credit_price: float = None, service_type: str = None) -> float:
    """Convert credits to dollars using configured credit price.

    When service_type is provided, uses multi-rate pricing for accurate
    dollarization across compute, serverless, and AI services.
    """
    if service_type:
        try:
            from .credit_rates import credits_to_dollars_multi_rate
            return credits_to_dollars_multi_rate(credits, service_type)
        except Exception:
            pass
    if credit_price is None:
        credit_price = get_credit_price()
